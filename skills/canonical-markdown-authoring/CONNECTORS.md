...
| Node toolchain | `~~node` | Node.js >= 20 with `npm` | `pnpm`/`yarn` work for the install step but `npm run generate:employment-templates` is the canonical entry point |
...
This regenerates `scripts/template-specs/<id>.json` and
`content/templates/<id>/template.docx` from the canonical source.
...
`source_json`. Run it before committing.
