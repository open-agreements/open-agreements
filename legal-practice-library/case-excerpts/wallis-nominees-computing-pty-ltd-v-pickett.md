---
type: Case Excerpt
title: Wallis Nominees (Computing) Pty Ltd v Pickett
description: >-
  Wallis Nominees (Computing) Pty Ltd v Pickett [2013] VSCA 24 (Warren CJ and
  Davies AJA). — quoted in 1 document(s).
citation: >-
  Wallis Nominees (Computing) Pty Ltd v Pickett [2013] VSCA 24 (Warren CJ and
  Davies AJA).
resource: 'https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html'
timestamp: '2026-06-10'
document_references:
  - type: Practice Note
    resource: 'https://openagreements.org/practice-guides/non-compete/au/victoria'
tags:
  - case-law
  - non-compete
---

# Wallis Nominees (Computing) Pty Ltd v Pickett

*Wallis Nominees (Computing) Pty Ltd v Pickett [2013] VSCA 24 (Warren CJ and Davies AJA).*

Supporting case-law excerpt. Quoted across 1 document(s) in the OpenAgreements legal practice library.

## Quoted passages

### wallis-nominees-computing-pty-ltd-v-pickett-a-court-can-remove-words-from-986d4da5 {#wallis-nominees-computing-pty-ltd-v-pickett-a-court-can-remove-words-from-986d4da5}

> a court can remove words from a restraint clause but not rewrite it.

- supports: `cascading-clauses`
- source_cards: `wallis-remove-not-rewrite`
- cited_by: [Non-Compete Enforceability in Victoria](../non-compete/au/victoria.md)
- link_to_source: <https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html>

### wallis-nominees-computing-pty-ltd-v-pickett-the-assessment-of-whether-a-legitimate-1137f0a0 {#wallis-nominees-computing-pty-ltd-v-pickett-the-assessment-of-whether-a-legitimate-1137f0a0}

> The assessment of whether a legitimate interest exists is to be assessed at the time the contract is being entered into.

- supports: `garden-leave`, `reasonableness-factors`, `tolling`
- source_cards: `wallis-assessed-at-time`, `wallis-assessed-at-time-garden`, `wallis-assessed-at-time-tolling`
- cited_by: [Non-Compete Enforceability in Victoria](../non-compete/au/victoria.md)
- link_to_source: <https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html>

### wallis-nominees-computing-pty-ltd-v-pickett-the-commonly-used-form-of-a-dd1ee4d6 {#wallis-nominees-computing-pty-ltd-v-pickett-the-commonly-used-form-of-a-dd1ee4d6}

> the commonly used form of a cascading clause where it starts with the broadest possible restraint then provides increasingly narrow alternatives, each one standing alone.

- supports: `cascading-clauses`
- source_cards: `wallis-cascading`
- cited_by: [Non-Compete Enforceability in Victoria](../non-compete/au/victoria.md)
- link_to_source: <https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html>

### wallis-nominees-computing-pty-ltd-v-pickett-the-legitimate-interest-alleged-by-dws-4918d131 {#wallis-nominees-computing-pty-ltd-v-pickett-the-legitimate-interest-alleged-by-dws-4918d131}

> The legitimate interest alleged by DWS is its goodwill in its customers, the essence of which is its customer connections which may include knowledge of, and influence over, its customers.

- supports: `customer-non-solicitation`, `legitimate-interest`
- source_cards: `vic-cust-legitimate-interest`, `wallis-customer-connection`
- cited_by: [Non-Compete Enforceability in Victoria](../non-compete/au/victoria.md)
- link_to_source: <https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html>

### wallis-nominees-computing-pty-ltd-v-pickett-the-test-is-whether-the-restraint-ce2ef48b {#wallis-nominees-computing-pty-ltd-v-pickett-the-test-is-whether-the-restraint-ce2ef48b}

> The test is whether the restraint does more than what is reasonably necessary to protect the employer's legitimate interest.

- supports: `contractors-and-sale`, `garden-leave`, `legitimate-interest`
- source_cards: `wallis-test`, `wallis-test-contractors`, `wallis-test-garden`
- cited_by: [Non-Compete Enforceability in Victoria](../non-compete/au/victoria.md)
- link_to_source: <https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html>

### wallis-nominees-computing-pty-ltd-v-pickett-we-conclude-that-while-dws-did-51c223b5 {#wallis-nominees-computing-pty-ltd-v-pickett-we-conclude-that-while-dws-did-51c223b5}

> we conclude that while DWS did have a legitimate interest in restraining Mr Pickett, the clause was unreasonable having regard to both its extent and to its duration.

- supports: `customer-non-solicitation`, `legitimate-interest`, `reasonableness-factors`
- source_cards: `vic-cust-void-if-wider`, `wallis-conclude-unreasonable`, `wallis-conclude-unreasonable-scope`
- cited_by: [Non-Compete Enforceability in Victoria](../non-compete/au/victoria.md)
- link_to_source: <https://www.austlii.edu.au/cgi-bin/viewdoc/au/cases/vic/VSCA/2013/24.html>
