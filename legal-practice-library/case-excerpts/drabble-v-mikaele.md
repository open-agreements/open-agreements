---
type: Case Excerpt
title: Drabble v. Mikaele
description: Drabble v. Mikaele (Land & Titles Div. 2004). — quoted in 1 document(s).
citation: Drabble v. Mikaele (Land & Titles Div. 2004).
resource: 'https://asbar.org/case-law/drabble-v-mikaele/'
timestamp: '2026-06-03'
document_references:
  - type: State Law Practice Note
    resource: 'https://openagreements.org/practice-guides/non-compete/us/american-samoa'
tags:
  - case-law
  - non-compete
---

# Drabble v. Mikaele

*Drabble v. Mikaele (Land & Titles Div. 2004).*

Supporting case-law excerpt. Quoted across 1 document(s) in the OpenAgreements legal practice library.

## Quoted passages

### drabble-v-mikaele-for-a-preliminary-injunction-to-issue-2b080e29 {#drabble-v-mikaele-for-a-preliminary-injunction-to-issue-2b080e29}

> For a preliminary injunction to issue, the applicant must show a substantial likelihood that he will prevail at trial on the merits and obtain a permanent injunction, and that he will suffer great or irreparable harm before a full and final trial can be fairly held on the requested permanent injunction.

- supports: `remedies`
- source_cards: `drabble-standard`
- cited_by: [Non-Competes in American Samoa](../non-compete/us/american-samoa.md)
- link_to_source: <https://asbar.org/case-law/drabble-v-mikaele/>
