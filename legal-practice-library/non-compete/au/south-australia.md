...
description: >-
  In South Australia a post-employment non-compete is presumptively void as a
  restraint of trade and binds a former employee only so far as it is
  reasonable; South Australia has no statutory read-down power, so courts apply
  the common-law blue-pencil rule only — they can delete severable words but
  cannot rewrite an overbroad clause, and a clause that severance cannot save
  fails entirely.
