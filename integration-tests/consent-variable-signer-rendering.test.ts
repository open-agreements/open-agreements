...
describe('SAFE consent variable signer rendering', () => {
  for (const signerCount of [1, 3, 7]) {
    it.openspec('OA-FIL-024')(
      `renders board consent with exactly ${signerCount} signature blocks from JSON template output`,
      async () => {
    ...
    it.openspec('OA-FIL-024')(
      `renders stockholder consent with exactly ${signerCount} signature blocks from JSON template output`,
      async () => {
