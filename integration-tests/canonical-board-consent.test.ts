...
const TEMPLATE_DIR = findTemplateDir('openagreements-board-consent-safe');
if (!TEMPLATE_DIR) throw new Error('openagreements-board-consent-safe template not found');
...
describe('Canonical SAFE board consent (traditional)', () => {
  it('compiles canonical board consent source and metadata', () => {
  ...
  it('renders DOCX from the canonical board consent source with traditional structure', async () => {
  ...
  it('fills board consent without leaving signer loop markers', async () => {
  ...
  it('rejects fills with empty board_members', async () => {
  ...
  it('rejects fills missing board_members entirely', async () => {
  ...
  it('renders Pages-compatible explicit-style tree (board)', async () => {
    const style = loadStyleProfile(STYLE_PATH);
    const compiled = compileCanonicalSourceFile(SOURCE_PATH);
    const rendered = renderFromValidatedSpec(compiled.contractSpec, style);
    const buffer = await Packer.toBuffer(rendered.document);
