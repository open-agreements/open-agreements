...
- `postconditions.ts` — produce new `VerifyCheck`s (`VerifyResult = {passed, checks[]}`, `VerifyCheck = {name, passed, details?}`). Phase-1: `no_unresolved_placeholder`, `all_occurrences_identical`, `no_double_dollar`. `no_double_dollar` is **currency-class**: authored only on monetary fields (`purchase_price_per_share`, `par_value_per_share`), never on name/enum/date fields where a `$ $`/`$$` artifact cannot occur.
