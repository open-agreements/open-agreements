...
### Requirement: Computed Profile Evaluation
A recipe MAY include a `computed.json` file defining conditional rules that derive
fill values and audit metadata from user inputs. The evaluator SHALL execute rules
in a multi-pass loop until values stabilize or `max_passes` is reached.

#### Scenario: [OA-062] Multi-pass computed profile evaluation stabilizes deterministically
- **GIVEN** a computed profile with chained conditional rules (e.g., dispute-resolution mode → track → default district → alignment)
- **WHEN** the evaluator runs with concrete input values
- **THEN** it stabilizes within `max_passes`, produces expected fill values and audit metadata, and pairwise reduction covers the interaction space without full Cartesian explosion

#### Scenario: [OA-066] Dispute-resolution invariants hold across input combinations
- **GIVEN** the NVCA SPA computed profile with dispute-resolution and governing-law rules
- **WHEN** tested via MC/DC-style toggling, deterministic pseudo-random sampling, and metamorphic mode-switching
- **THEN** dispute-resolution track always matches the selected mode, governing-law baseline is stable, forum-alignment reflects state/mode interaction, and default fill values are consistent with the selected jurisdiction
