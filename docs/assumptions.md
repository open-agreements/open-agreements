...
Use this together with `tests/ASSUMPTION_TEST_MATRIX.md`.
...
- Validation: `tests/nvca-assumptions.test.ts` (`clean step preserves bracket-prefixed headings while removing bracketed alternatives`).
...
- Validation: `tests/nvca-assumptions.test.ts` (same test as OA-ASSUMP-001; validates both “keep headings” and “drop alternatives”).
...
- Validation: `tests/bracket-normalizer.test.ts` and `tests/nvca-assumptions.test.ts`.
...
- Validation: `tests/nvca-assumptions.test.ts` (`declarative normalize strips heading-leading brackets and trims unmatched trailing brackets`).
...
- Validation: `tests/nvca-assumptions.test.ts` (expects headings fixed but option pair still present unless separately configured).
...
- Validation: `tests/scan-metadata-completeness.test.ts`.
...
- Validation: `tests/formatting-boundary-diff.test.ts`.
...
- Validation: `tests/source-drift-canary.test.ts` and `npm run check:source-drift`.
...
- Validation: `tests/nvca-option-policy.test.ts`.
...
- Validation: `tests/bracket-normalizer.test.ts`, `tests/source-drift-canary.test.ts`, and `npm run check:source-drift`.

## Quick verification commands

```bash
# Assumption-focused regression checks
npx vitest run tests/nvca-assumptions.test.ts tests/bracket-normalizer.test.ts
