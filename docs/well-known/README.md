...
> **Build note (updated).** The in-repo Eleventy site and its `.well-known` generators (`scripts/build-wellknown-ai-index.cjs`, `scripts/sign-manifest.cjs`) were removed in #575. The `.well-known/*` files are now **static, committed artifacts** at the repo root under `.well-known/` — nothing in this repo regenerates them. Historical "Generated by …" references below describe how `ai/index.json` and `arp/index.sig` were produced under the old Eleventy build; regenerating/signing them, if needed, now happens out of band or in the deploy repo.
