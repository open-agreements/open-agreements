...
- Generated build artifacts currently include `/.well-known/ai/index.json`, `/.well-known/arp/index.sig`, and `/.well-known/agent-skills/*`.
...
| `/.well-known/agent-skills/index.json` | No — `agent-skills` is the current agentskills.io discovery path | Generated by `scripts/generate_site_indexes.mjs` into `_site/.well-known/agent-skills/index.json` plus sibling ZIP archives | Add generated skills CLI discovery surface |
...
- Generator: `scripts/generate_site_indexes.mjs`, which writes `_site/.well-known/agent-skills/index.json` and one sibling ZIP archive per public skill.
- Header / Content-Type expectations: No explicit `headers` rule in `vercel.json`; as generated `.json` and `.zip` files they are expected to be served with platform-default static-file headers.
- Expected consumers: The `skills` CLI and compatible agent tooling installing OpenAgreements skills from `npx skills add openagreements.org`.
