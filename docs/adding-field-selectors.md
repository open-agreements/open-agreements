...
| `removeHeaderFooterDrawings` | boolean | Strip `<w:drawing>`/`<w:pict>` runs from header/footer parts whose referenced media exceeds the size threshold, plus the orphaned relationships and media parts. Catches image-only content (e.g. a full-page "how to use this template" screenshot anchored in a header) that text-anchored mechanisms cannot see. Small spacer images stay untouched. **Byte size is a coarse heuristic, not semantic image classification**: a legitimate logo or artwork larger than the threshold is indistinguishable from an instructions screenshot and will be removed by design. Before enabling, inspect the template's `word/media/` parts (`unzip -l template.docx 'word/media/*'`) and set a template-specific `headerFooterDrawingMinBytes` if the default would catch artwork you want to keep. |
| `headerFooterDrawingMinBytes` | number | Size threshold (bytes) for `removeHeaderFooterDrawings`. Defaults to 51200 (50 KB) — well above the ~146-byte spacer PNGs common in Google-Docs exports, well below instructions-page screenshots (~125 KB+) — and matches legal-explainer's source-side vendored-docx audit threshold. Tune per template when its legitimate header/footer artwork exceeds the default. |
