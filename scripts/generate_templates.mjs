...
async function writeDocx(doc, outPath) {
  ...
  writeFileSync(resolve(outPath), await postProcessGeneratedDocx(buf));
}
...
async function main() {
  ...
  const repairedTemplateIds = await postProcessTemplateDocxFiles();
  if (repairedTemplateIds.length > 0) {
    console.log(`Post-processed DOCX structure for ${repairedTemplateIds.join(', ')}.`);
  }
